...
export class OverviewComponent
  ...
  ngOnInit() {
    ...
    if (!!objInvestorProfileFormOne) {
      this.objOverview = objInvestorProfileFormOne.get(
        'objOverview'
      ) as FormGroup;
      if (!!this.objOverview.value.objLogo) {
        if (!!this.objOverview.value.objLogo.data) {
          this.strUploadData = this.objOverview.value.objLogo.data;
        }
        if (!!this.objOverview.value.objLogo.fileName) {
          this.strLogoName = this.objOverview.value.objLogo.fileName;
        }
      }
    }
  }
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (!!changes.blnEdit.currentValue) {
    }
  }
  ...
  get overviewFormControl() {
    ...
    return objInvestorProfileFormOne.get('objOverview')['controls'];
  }
  /**
   * Update form
   */
